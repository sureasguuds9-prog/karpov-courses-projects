# Project 3 — подробный разбор решения
...
В Project 3 по проверенным признакам значимых различий не найдено. Это не
...
```python
conversion = experiment.groupby("group")["payer"].mean()
```
...
```python
metrics_plot["conversion"].plot.bar()
```

На графике конверсии ось начинаем с нуля. Иначе небольшая разница визуально
может выглядеть огромной.

Для денежных метрик отдельно строим ARPU и Premium ARPU. Так видно, что тестовый
...
## 12. Проверка ARPU через Welch t-test
...
comparison_metrics["conversion"] = (
    comparison_metrics["payers"]
    / comparison_metrics["users"]
)

comparison_metrics["premium_conversion"] = (
    comparison_metrics["premium_buyers"]
    / comparison_metrics["users"]
)

comparison_metrics["trial_conversion"] = (
    comparison_metrics["trial_buyers"]
    / comparison_metrics["users"]
)
```

Если обратиться к несуществующей колонке, например `premium_conversion` до её
...
## 14. Как интерпретировать результат Project 3
...
| Premium ARPU | 177.13 | 188.19 | +6.24% |
...
- Premium ARPU: `p = 0.8117` — рост не подтверждён;
...
В Project 3 даты дают невозможную хронологию при обычном парсинге. Пока формат
...
4. «Основные метрики — payer CR, premium CR и ARPU; ARPPU использовал как
   диагностическую».
5. «Для долей применил z-тест пропорций, для средних — Welch t-test».
